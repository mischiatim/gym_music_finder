...
def compute_features_from_filename(wav_filename,srate,max_len=20):
#Here I just modified the 'compute_features' function of FMA to take 
#directly the filename and sampling rate as argument rather than the track id,
        
    track_title = wav_filename.split(sep='.wav')[0][:max_len]

    features = pd.Series(index=columns(), dtype=np.float32, name=track_title)

    #features = pd.Series(index=columns(), dtype=np.float32, name=tid)

    #I had to change the following because it was making the code crash (due to incompatibility with numpy version)
    ...
    try:
        #filepath = utils.get_audio_path(os.environ.get('AUDIO_DIR'), tid)
        #x, sr = librosa.load(wav_filename, sr=None, mono=True)  # kaiser_fast
        x, sr = librosa.load(wav_filename, sr=srate, mono=True)  # kaiser_fast

        f = librosa.feature.zero_crossing_rate(x, frame_length=2048, hop_length=512)
        feature_stats('zcr', f)

        cqt = np.abs(librosa.cqt(x, sr=srate, hop_length=512, bins_per_octave=12,
                                 n_bins=7*12, tuning=None))
        assert cqt.shape[0] == 7 * 12
        assert np.ceil(len(x)/512) <= cqt.shape[1] <= np.ceil(len(x)/512)+1
        ...
        mel = librosa.feature.melspectrogram(sr=srate, S=stft**2)
        del stft
        f = librosa.feature.mfcc(S=librosa.power_to_db(mel), n_mfcc=20)
        feature_stats('mfcc', f)

    except Exception as e:
        print('{}: {}'.format(track_title, repr(e)))
